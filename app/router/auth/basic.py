# from typing import Annotated
# from fastapi.security import HTTPBasic ,HTTPBasicCredentials
# from fastapi import APIRouter,HTTPException,Depends
# from app.database import db_dep
# from app.utils import verify_password
# from app.models import User
# from sqlalchemy import select

# basic=HTTPBasic()

# router=APIRouter(
#     prefix="/login",
# )

# @router.post("/",status_code=400)
# async def login_user (
#     session:db_dep,
#     credentionals:Annotated[HTTPBasicCredentials,Depends(basic)]):
#     stmt=select(User).where(User.email==credentionals.username)
#     res=session.execute(stmt).scalars().first()
    
#     if not res:
#         raise HTTPException(
#             status_code=404,
#             detail="User Not Found"
#         )
#     if not verify_password(credentionals.password,res.password_hash):
#         raise HTTPException(
#             status_code=401,
#             detail="Invalid password"
#         )
#     return res

from fastapi import APIRouter

from app.database import db_dep
from app.dependencies import current_user_basic_dep
from app.schemas import (
    UserProfileResponse,
    UserProfilRequest,
)


router = APIRouter(prefix="/basic_auth")


@router.get("/profile/", response_model=UserProfileResponse)
async def user_profile(db: db_dep, current_user: current_user_basic_dep):
    return current_user


@router.put("/profile/", response_model=UserProfileResponse)
async def user_profile_update(
    db: db_dep,
    current_user: current_user_basic_dep,
    update_data: UserProfilRequest,
):
    for attr, value in update_data.model_dump(exclude_unset=True).items():
        setattr(current_user, attr, value)

    db.commit()
    db.refresh(current_user)

    return current_user


@router.delete("/profile/", status_code=204)
async def profile_delete(db: db_dep, current_user: current_user_basic_dep):
    current_user.is_active = False
    current_user.is_deleted = True
    current_user.deleted_email = current_user.email
    current_user.email = None

    db.commit()